...
var tests = []test{
	...
	// bad paths
	{"/bookstore", errorResult("xmltree: paths cannot be absolute.")},
	{"./bookstore/book[]", errorResult("xmltree: path contains an empty filter expression.")},
	{"./bookstore/book[@category='WEB'", errorResult("xmltree: path has invalid filter [brackets].")},
	{"./bookstore/book[@category='WEB]", errorResult("xmltree: path has mismatched filter quotes.")},
	{"./bookstore/book[author]a", errorResult("xmltree: path has invalid filter [brackets].")},
}
...
func fail(t *testing.T, test test) {
	t.Errorf("xmltree: failed test '%s'\n", test.path)
}
