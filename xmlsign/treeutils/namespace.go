...
func (ctx NSContext) declare(prefix, namespace string) xmltree.Attr {
...
type NSIterHandler func(NSContext, *xmltree.Element) error

// NSTraverse traverses an element tree, invoking the passed handler for each element
// in the tree.
func NSTraverse(ctx NSContext, el *xmltree.Element, handle NSIterHandler) error {
...
func NSDetatch(ctx NSContext, el *xmltree.Element) (*xmltree.Element, error) {
	ctx, err := ctx.SubContext(el)
	if err != nil {
		return nil, err
	}
	...
	attrs := make([]xmltree.Attr, 0, len(el.Attr))
...
// surrounding context.
func NSSelectOne(el *xmltree.Element, namespace, tag string) (*xmltree.Element, error) {
...
// and tag. If such an element is found, a new *xmltree.Element is returned which is a
// copy of the found element, but with all in-context namespace declarations attached
// to the element as attributes.
func NSSelectOneCtx(ctx NSContext, el *xmltree.Element, namespace, tag string) (*xmltree.Element, error) {
	var found *xmltree.Element

	err := NSFindIterateCtx(ctx, el, namespace, tag, func(ctx NSContext, el *xmltree.Element) error {
		var err error
...
func NSFindIterate(el *xmltree.Element, namespace, tag string, handle NSIterHandler) error {
...
func NSFindIterateCtx(ctx NSContext, el *xmltree.Element, namespace, tag string, handle NSIterHandler) error {
	err := NSTraverse(ctx, el, func(ctx NSContext, el *xmltree.Element) error {
...
// context.
func NSFindOne(el *xmltree.Element, namespace, tag string) (*xmltree.Element, error) {
...
func NSFindOneCtx(ctx NSContext, el *xmltree.Element, namespace, tag string) (*xmltree.Element, error) {
	var found *xmltree.Element
...
// is returned.
func NSBuildParentContext(el *xmltree.Element) (NSContext, error) {
	parent := el.Parent()
	if parent == nil {
